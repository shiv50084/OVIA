...
extern "C" {
    ...
    static void AIFWriteCOMM(OVIA *Ovia, BitBuffer *BitB) {
        ...
        WriteBits(MSByteFirst, LSBitFirst, BitB, 16, OVIA_GetNumChannels(Ovia)); // 2
        WriteBits(MSByteFirst, LSBitFirst, BitB, 32, OVIA_GetNumSamples(Ovia)); // 7979748
        WriteBits(MSByteFirst, LSBitFirst, BitB, 16, OVIA_GetBitDepth(Ovia)); // 16
        ...
        WriteBits(MSByteFirst, LSBitFirst, BitB, 16, (SampleRate >> 52) + 15360);
    ...
    static void AIFWriteArtist(OVIA *Ovia, BitBuffer *BitB) {
        ...
        if (ArtistTagIndex != 0xFFFFFFFFFFFFFFFF) {
            ...
            uint32_t TagByte = 0ULL;
            do {
                WriteBits(MSByteFirst, LSBitFirst, BitB, 8, ArtistTag[TagByte]);
                TagByte += 1;
    ...
    static void AIFWriteSSND(OVIA *Ovia, BitBuffer *BitB) {
        if (Ovia != NULL && BitB != NULL) {
            ...
            WriteBits(MSByteFirst, LSBitFirst, BitB, 32, Offset);
            WriteBits(MSByteFirst, LSBitFirst, BitB, 32, BlockSize);
        } else if (Ovia == NULL) {
    ...
    void AIFWriteHeader(OVIA *Ovia, BitBuffer *BitB) {
        if (Ovia != NULL && BitB != NULL) {
            ...
            WriteBits(MSByteFirst, LSBitFirst, BitB, 32, ((NumSamples * NumChannels) * Bits2Bytes(BitDepth, true))); // FIXME: AIF Size calculation is wrong
            ...
            if (AudioType == AudioContainer_SInteger8) {
                ...
                for (uint64_t Sample = 0; Sample < NumSamples; Sample++) {
                    for (uint64_t Channel = 0; Channel < NumChannels; Channel++) {
                        WriteBits(MSByteFirst, LSBitFirst, BitB, BitDepth, Samples[Channel][Sample]);
